...
public class Task2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < equations.size(); i++) {
            ...
            findDots(equations.get(i), derEquations.get(i), i);
        }
        ...
        task21.add((x, y, a, alpha, betta) -> (1 / Math.pow(Math.cos(x * y + a), 2)) * y - 2 * x); // производная по х
        task21.add((x, y, a, alpha, betta) -> (1 / Math.pow(Math.cos(x * y + a), 2)) * x); // производная по y
        task21.add((x, y, a, alpha, betta) -> 2 * x * alpha); // x
        task21.add((x, y, a, alpha, betta) -> 2 * y * betta); // y

        LinkedList<Dot> dots = new LinkedList<>();
        dots.add(new Dot(0.2, 0.6, 2, 0.8, 0.5, 0.1428));
        dots.add(new Dot(0.4, 0.8, 2, -0.3, 0.6, 0.805));
        dots.add(new Dot(0.3, 0.2, 3, 1, 0.5, 0.0357));
        dots.add(new Dot(0, 0.6, 2, 0.6, 0.6, 0.473));
        ...
        dots2.add(new Dot(0, 0, 0, -0.5, 1, 1));
    ...
    private static void approximation(Operationable equation, Operationable derEquation, double x, double lambda) {
        double lyambda = 0.01; //шаг приближения к формуле
        double x0;
        int n = 0;
        do {
            System.out.println("Итерация №" + ++n);
            x0 = x;
            x = x0 - lyambda * equation.calculate(x0);
            System.out.println("x = " + x);
        } while ((Math.abs(x - x0)) > EPS);
        System.out.println("\nРешение: x = " + x);
    }

    private static void findDots(Operationable equation, Operationable derEquation, double lambda) {
        int dotCnt = 1;
        for (double i = minRange; i <= maxRange; i += 0.5) {
            if (equation.calculate(i) * equation.calculate(i + 0.5) < 0 || equation.calculate(i) == 0) { //в плюсе
                System.out.println(dotCnt++ + " корень:");
                approximation(equation, derEquation, i, i);
            }
        }
    }
    ...
    public static void newton(List<OperationableNewton> data, Dot dot) {
        ...
        do {
            ...
            double detJ = (detFunc1x * detFunc2y) - (detFunc1y * detFunc2x); //перемножаем на значения функций
            x1 = x - (detA1 / detJ);
            y1 = y - (detA2 / detJ);
            iterCnt++;
        } while (Math.sqrt(Math.pow(f1.calculate(x1, y1, a2, alpha, betta), 2) + Math.pow(f2.calculate(x1, y1, a2, alpha, betta), 2)) > EPS &&
                Math.sqrt(Math.pow(x1 - x, 2) + Math.pow(y1 - y, 2)) > EPS); // критерий остановки
    ...
    static class Dot {
        double x, y, a, alpha, betta, lambda;

        public Dot(double a, double alpha, double betta, double x, double y, double lambda) {
            this.a = a;
            this.alpha = alpha;
            this.betta = betta;
            this.x = x;
